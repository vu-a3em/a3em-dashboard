 ...
 * with no connectivity, and a map that renders as grey squares exactly when it matters
  ...
  return (
    <Pane
      ...
      note={`${track.fixes.length} ${track.fixes.length === 1 ? 'fix' : 'fixes'} · ${
        track.stationary ? 'stationary' : formatDistance(track.displacementMetres)
      }`}
    >
      <p className="hint">
        {track.stationary
          ? 'The device stayed put. The spread below is receiver error, not movement — note the scale.'
          : `The device moved ${formatDistance(track.displacementMetres)} between its first and last fix.`}
      </p>
      ...
      <div className="track-plot">
        ...
        {bar ? (
          <div className="track-scale">
            ...
            <span>{formatDistance(bar.metres)}</span>
          </div>
        ) : null}
      </div>
      ...
      <div className="grid stats" style={{ marginTop: 14 }}>
        ...
        <Stat label="Displacement" value={formatDistance(track.displacementMetres)} />
        <Stat label="Path length" value={formatDistance(track.pathMetres)} />
        <Stat
          label="Centre"
          value={`${track.bounds.centreLatitude.toFixed(4)}, ${track.bounds.centreLongitude.toFixed(4)}`}
        />
      </div>
