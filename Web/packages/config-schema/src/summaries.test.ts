...
describe('pane summaries', () => {
  it('names the device by its label, microphone and gain', () => {
    ...
    assert.equal(summariseDevice(config), 'BEAR-04 · digital mic at 1.5 dB · cutoff 3.25 V');
  });
  ...
  it('leaves the cutoff out when the device records to exhaustion', () => {
    ...
    assert.doesNotMatch(summariseDevice(config), /cutoff/);
  });

  it('says so rather than showing an empty label', () => {
    const config = defaultConfig('UTC');
    config.deviceLabel = '   ';
    assert.match(summariseDevice(config), /^No label · /);
  });
  ...
  it('reduces the dates to a length', () => {
    ...
    assert.equal(summariseSchedule(config), '14 days · America/Chicago');
  });

  it('counts phases only when the deployment uses them', () => {
    const config = defaultConfig('UTC');
    assert.equal(summarisePhases(config), 'Single phase');
    config.isPhased = true;
    config.phases = [defaultPhase(), defaultPhase(), defaultPhase()];
    assert.equal(summarisePhases(config), '3 phases');
  });
  ...
  it('gives the audio mode, rate and clip length, and flags Opus', () => {
    ...
    assert.equal(summariseAudio(phase), 'Continuous · 16 kHz · 10 s clips');
    assert.equal(summariseAudio({ ...phase, useOpusEncoding: true }), 'Continuous · 16 kHz · 10 s clips · Opus');
  });

  it('drops the motion rate when motion recording is off', () => {
    const phase = defaultPhase();
    assert.equal(summariseMotion({ ...phase, imuRecordingMode: 'NONE' }), 'Disabled');
    assert.equal(
      summariseMotion({ ...phase, imuRecordingMode: 'AUDIO', imuSampleRateHz: 50 }),
      'Synchronised with audio · 50 Hz',
    );
  });
  ...
  it('names only the corners the filter type actually uses', () => {
    ...
    assert.equal(summariseFilter({ ...phase, audioFilterType: 'NONE' }), 'No filtering');
    assert.equal(summariseFilter({ ...phase, audioFilterType: 'HIGH' }), 'High-pass 350 Hz');
    assert.equal(summariseFilter({ ...phase, audioFilterType: 'LOW' }), 'Low-pass 7.8 kHz');
    assert.equal(summariseFilter({ ...phase, audioFilterType: 'BAND' }), 'Band 350 Hz – 7.8 kHz');
  });
  ...
  it('leaves the silence band out while the threshold is zero', () => {
    ...
    assert.equal(summariseSilence({ ...phase, silenceThreshold: 0 }), 'Off');
    assert.equal(summariseSilence({ ...phase, silenceThreshold: 0.2 }), '20% · 300 Hz – 7.8 kHz');
  });

  it('puts errors before warnings and says when there are neither', () => {
    assert.equal(summariseReadiness({ errors: 0, warnings: 0 }), 'Ready to write');
    assert.equal(summariseReadiness({ errors: 0, warnings: 1 }), '1 warning');
    assert.equal(summariseReadiness({ errors: 2, warnings: 0 }), '2 errors');
    assert.equal(summariseReadiness({ errors: 1, warnings: 3 }), '1 error · 3 warnings');
  });
});
